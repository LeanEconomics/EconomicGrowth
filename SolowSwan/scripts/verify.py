...
def main():
    ...
    root_imports = {line.removeprefix("import ").replace(".", "/") + ".lean"
                    for line in (ROOT / "Solow1956.lean").read_text().splitlines()
                    if line.startswith("import Solow1956.")}
    ...
    for module in config["modules"]:
        ...
        for line in clean.splitlines():
            ...
            if line.startswith("namespace "):
                ...
                namespaces.pop()
            elif match := re.match(r"(?:@\[[^]]*\] )?(?:noncomputable )?(theorem|lemma|def|abbrev|structure) ([\w.']+)", line):
        ...
        for line in raw.splitlines():
            if line.startswith("import "):
                if not line.startswith("import Solow1956") and line not in imports:
                    imports.append(line)
            else:
                body.append(line)
        bodies.append("\n".join(body))
    ...
    for result in results:
        ...
        for certificate in result["certificates"]:
            ...
            certificate_count += 1
    assert len(results) == environment["cases"] == 10
    assert certificate_count == environment["certificates"] == 20
    for name in ("Solow1956.lean", "lean-toolchain", "lakefile.toml",
